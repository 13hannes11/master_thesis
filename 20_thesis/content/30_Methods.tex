...
\begin{table}[ht]
    \centering
    \caption{Comparison of datasets used for each model by magnification, pixel size, input size and dataset size.}
    \begin{tabular}{crrrr} 
        \hline
...
\begin{figure}[tb]
    ...
    \caption{Patches that are considered to be in focus for the best image selection task. The image on the left shows the background most in focus while the one on the right the least. Different parts of the egg are fairly well in focus in each of the images.}
