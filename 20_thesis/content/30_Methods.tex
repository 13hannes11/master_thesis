...
\todo[inline]{add something about focus values, what they mean and magnitude (range is [0, 1023] normalized to [0, 1]), 
focus range of microscopy: \unit{4.5}{\milli\meter} to \unit{5.78}{\milli\meter} => \unit{1.28}{\milli\meter}

=> in range [0, 1023] one unit = \unit{0.00125}{\milli\meter} = \unit{1.25}{\micro\meter}
=> in range [0, 1], 1 = \unit{1280}{\micro\meter}
...
The \acp{resnet} models use predefined \acp{resnet} models from PyTorch. However, the fully connected layer is changed to provide only a single regression output. Another, hyperparameter is the use of a pretrained model trained on ImageNet \todo{cite dataset} to initialize the weights of a model.

All presented \ac{resnet} models use pretrained models.

\todo{resnet presented model parameters}
...
\Acs{poop}-\acs{cnn} and \acs{poop}-\acs{fc} are trained using separate runs of Optuna \todo{reference Optuna}. In these runs the architecture and hyperparameters are optimized.
