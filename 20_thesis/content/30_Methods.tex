...
The \acp{resnet} models use predefined \acp{resnet} models from PyTorch. However, the fully connected layer is changed to provide only a single regression output. Another, hyperparameter is the use of a pretrained model trained on ImageNet \todo{cite dataset} to initialize the weights of a model. All presented \ac{resnet} models use transfer learning, however, their learning rate varies. The resulting hyperparameters are shown in \autoref{tab:Methods:Models:Comparison}.
...
\begin{table}[ht]
    ...
    \begin{tabular}{ l c c }
        ...
        \hline
        \acp{resnet}-18  & 0.0036 & true \\
        \acp{resnet}-34  & 0.0010 & true \\
        \acp{resnet}-101 & 0.0012 & true \\
    \end{tabular}
