...
The \ac{plcc} \todo{add citation of paper that introduces it} is a correlation measure used as another metric to measures if relationship between two variables is linear \cite{profillidis2019statistical}. A linear relationship is expected between the actual focus distance and the prediction of model. It is defined as:
...
The \ac{srcc} \todo{citation of introduction} is related to the \ac{plcc}, however it uses the rank of variables instead of their actual value. Therefore, it describes the relationship between two variables using monotonic function \todo{source}. The formula that describes \ac{srcc} looks as follows:
...
\todo{explain of these metric which are used in related work and where}
