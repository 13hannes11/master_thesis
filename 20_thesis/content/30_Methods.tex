...
At the time of writing, there is no available dataset for Kato-Katz prepared slides, especially, not of \acp{ntd}. A large dataset of annotated slides that contain annotations for bounding boxes and the type of eggs is available at Etteplan. This dataset is subdivided into different studies and study 31 was used for this thesis. This study was performed in Uganda and contains a mix of slides. The majority of slides were scanned one month after preparation. A portion was scanned within 24 hours and a few were scanned directly after preparation.
...
The \ac{plcc} \cite{pearson1909determination} is a correlation measure used as another metric to measure if the relationship between two variables is linear \cite{profillidis2019statistical}. A linear relationship is expected between the actual focus distance and the prediction of the model. It is defined as:

\begin{equation} \wvar{\acs{plcc}} = \frac{\wfunc{cov}(X,Y)}{\wfunc{std}(X)  \wfunc{std}(Y)} \texttt{,} \end{equation}
with $\wfunc{cov}$ being the covariance and $\wfunc{std}$ being the \acl{std}. The value range of \ac{plcc} ranges from $-1$ to $1$ and according to \textcite{profillidis2019statistical} $\acs{plcc} = 1$ means a perfect positive correlation, $\ac{plcc} > 0.8$ a strong correlation. A $\ac{plcc} = 0$ means no correlation at all and values below zero indicate a negative correlation with $\ac{plcc} = -1$ being a perfectly negative correlation.


The \ac{srcc} \cite{spearman1904proof} is related to the \ac{plcc}, however, uses the rank of variables instead of their actual value. Therefore, it describes the relationship between two variables using a monotonic function. The formula that describes \ac{srcc} looks as follows:

\begin{equation} \wvar{\acs{srcc}} = \frac{\wfunc{cov}(\func{R}(X),R(Y))}{\wfunc{std}(\func{R(}X)) \cdot \wfunc{std}(\func{R}(Y))} \texttt{,} \end{equation} with $R$ the rank of variables, $\wfunc{std}$ the \acl{std}, and $\wfunc{cov}$ being the covariance.
