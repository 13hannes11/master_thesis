...
The worst performing model is \acs{poop}-\acs{fc} with most metrics lying below $0.04$ except \ac{srcc} with a value of $0.12$. This model shows no direct correlation with the focus input data. The \acs{poop}-\acs{cnn}'s \ac{mae} is only slightly better, but it shows some correlation for \ac{srcc} and \ac{plcc}, however the \ac{r2} score is fairly low with only $0.14$. 
