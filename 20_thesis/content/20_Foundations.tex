...
\subsection{Examination (Whole-Slide Imaging)}
...
\emph{Batch normalization} \cite{ioffe2015batch} is a layer only used during training (not during validation and testing) that normalizes the output $y$ of a layer before feeding it to the activation function. The normalization is done over a whole mini-batch. The mean is subtracted from the output and then a division by the standard deviation is made: $\frac{y - mean(y)}{std(y)}$. It allows a higher learning rate and less careful initialization, thereby, reducing the time it takes to train a network.
