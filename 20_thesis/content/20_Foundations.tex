...
\subsection{Soil-Transmitted Helminths} % Soil-Transmitted Helminths
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node[weights,below of=left] (0) {$b$};
            \path[draw,->] (0) -- (left);
    \end{tikzpicture}
    \caption[Perceptron visualization]{Visualization of a perceptron. The inputs $x_i$ are multiplied with their corresponding weights $w_i$. The results are summed up, and a bias is added. This is followed by the application of a threshold function. }
...
The small constant $\const{\epsilon}$ is used to prevent division by zero. \textcite{kingma2017adam} state that \ac{adam} is an optimizer that is fairly robust to noise.
